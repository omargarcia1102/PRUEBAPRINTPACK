...
def login():
    ...
    cursor.execute("SELECT * FROM usuarios WHERE usuario = %s AND estado = 'activo'", (usuario,))
...
def crear_producto():
    ...
    """, (
        id_nuevo, d.get('nombre'), session.get('usuario'),
        "Producto agregado al inventario", 0, d.get('stock')
    ))
...
def actualizar_producto(id):
    ...
    """, (
        id, nombre_producto, session.get('usuario'),
        "Producto editado", stock_anterior, d.get('stock')
    ))
...
def eliminar_producto(id):
    ...
    try:
        ...
        """, (
            id, nombre_producto, session.get('usuario'),
            "Producto eliminado del inventario", stock_actual, 0
        ))
...
def crear_venta():
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s)
        """, (numero_factura, id_cliente, session.get('usuario'), total, notas))
        ...
        for item in items:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, 'ELIMINAR', %s, %s, %s, %s)
            """, (
                id_producto, nombre_producto, session.get('usuario'),
                f"Venta #{numero_factura}", producto['stock'], producto['stock'] - cantidad
            ))
...
def get_venta_detalle(id):
    ...
    cursor.execute("SELECT * FROM detalle_ventas WHERE id_venta = %s", (id,))
...
def crear_cliente():
    ...
    try:
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (d.get('nombre'), d.get('email'), d.get('telefono'), d.get('empresa')))
        mysql.connection.commit()
        cursor.close()
        return jsonify({'ok': True}), 201
    except Exception as e:
...
def actualizar_cliente(id):
    ...
    try:
        cursor.execute("""
            ...
            WHERE id=%s
        """, (d.get('nombre'), d.get('email'), d.get('telefono'), d.get('empresa'), id))
        mysql.connection.commit()
        cursor.close()
